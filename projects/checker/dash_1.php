...
switch($_GET["action"]) {
	...
	case "add"://add item
		if(!empty($_POST["quantity"])) {
			...
			$itemArray = array($productByCode[0]["code"]=>array('name'=>$productByCode[0]["name"], 'code'=>$productByCode[0]["code"], 'quantity'=>$_POST["quantity"], 'item_regularprice'=>$productByCode[0]["item_regularprice"],'item_commission'=>$productByCode[0]["item_commission"]));
			//$itemArray = array($productByCode[0]["code"]=>array('name'=>$productByCode[0]["name"], 'code'=>$productByCode[0]["code"], 'quantity'=>1, 'item_regularprice'=>$productByCode[0]["item_regularprice"]));
	...
	break;
		...
		//if(!empty($_POST["quantity"])) {
			$productByCode = $db_handle->runQuery("SELECT * FROM menu_db WHERE code='" . $_GET["code"] . "'");
			$itemArray = array($productByCode[0]["code"]=>array('name'=>$productByCode[0]["name"], 'code'=>$productByCode[0]["code"], 'quantity'=>1, 'item_regularprice'=>$productByCode[0]["item_regularprice"],'item_commission'=>$productByCode[0]["item_commission"]));
...
?>
				<tr style="margin-top: 5px; margin-bottom:5px">
					...
					<td><center><a class="btn btn-info btn-lg" href="dash_1.php?action=minus&code=<?php echo $item["code"]; ?>"><span class="glyphicon glyphicon-minus-sign"></span></a></center></td>
					<td name="qty">
					
						<div><center><input type="number" min="1" name="quantity" id="<?php echo $item["code"]; ?>" value="<?php echo $item["quantity"]; ?>" size="2" onBlur="saveCart(this);" /></center></div>
							
					</td>
					<td>
					<center>
						<a class="btn btn-info btn-lg" href="dash_1.php?action=plus&code=<?php echo $item["code"]; ?>"?><span class="glyphicon glyphicon-plus-sign" ></span></a>
					</center>
					</td>
					
					<td name="itm"><?php echo "Php".$item["item_regularprice"]; ?></td>
		
					<td>
					</td>
					<td>
					</td>
					<td><a href="dash_1.php?action=remove&code=<?php echo $item["code"]; ?>" class="btnRemoveAction"><img src="remove_button.png" alt="Mountain View" style="width:30px;height:30px;"></a></td>
				</tr>
		<?php
        $item_total += ($item["item_regularprice"]*$item["quantity"]);
		}
...
//					header("Location: /amlfos/amlfos/checker/prototype2/prototype2/augment/apply/dash_1.php"); 
				} else {
		...
		foreach ($_SESSION["cart_item"] as $item){
			$subtotal = ($item["quantity"]*$item["item_regularprice"]);
		...
		VALUES ('".$item["quantity"]."','".$item["item_regularprice"]."','".$item["name"]."', '".$lastid."', '{$var_value}', NOW(),NOW(), '{$var_value1}', '{$var_value2}', '{$subtotal}',2);";
