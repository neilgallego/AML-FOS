...
FRAG;
																				include 'db.php';
																					$query = "SELECT SUM(served_tprice) as sum FROM served_db WHERE served_table = '$table_no'";
																					$query2 = "SELECT disc_bill, disc_total FROM discounts WHERE disc_id = 0";
																					...
																					if($r2){
																						...
																						while($row = mysqli_fetch_array($r)){ 
																							...
																							echo '<h3 id="total_bill" style="text-align:right;font-weight:bold;">' . 'Total Bill: ₱ ' . number_format($sum,2) . '</h3>';
																						}
																					}	
																					} else {
...
FRAG;
																	 ...
																	 echo'<form action="a.php" method="post" onsubmit="return confirm(Do you really want to submit?)">';
echo <<<FRAG
																	  				<div class="modal-dialog modal-md">
																						<div class="modal-content">
																							...
																							<div class="panel-body">
																								<div class="table-responsive">
																									...
																									<div class="modal-footer">
FRAG;
																										echo '<button type="submit" class="btn btn-primary" name="submitDiscount'.$table_no.'" onClick="return show_alert()">Apply Discount</button>';	
echo <<<FRAG
																									</div>			
																								</div>
																							</div>
...
FRAG;
																									if(isset($_POST['submitDiscount'.$table_no.''])){
																										if (isset($_POST['no_sc']) || isset($_POST['no_pax'])){
																											$no_sc = $_POST['no_sc'];
																											$no_pax = $_POST['no_pax'];
																													
																											$query = "INSERT INTO `discounts` (`disc_no_pax`, `disc_no_sc`,`disc_bill`) VALUES ('$no_sc', '$no_pax','$sum');";																																																																	
																											$query .= "update `discounts` set `disc_total` = (`disc_bill` - (((`disc_bill`/`disc_no_pax`)*.20)*`disc_no_sc`));";																																																																																																																																	
...
FRAG;
																									...
																									if(isset($_POST['submit'.$table_no.''])){
																										if (isset($_POST['cash_rcvd']) || isset($_POST['transac_no'])){																										
																											...
																											$query = "INSERT INTO `billing` (bill_time, bill_date, table_no, cash_rcvd, transac_no, total_bill) VALUES (now(), curdate(), '$table_no','$cash_rcvd', '$transac_no', '$sum');";																											
																											$query .= "update `billing` set `change` = `cash_rcvd` - `total_bill`;";																											
